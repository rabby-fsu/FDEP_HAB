...
def generate_hab_quotient_map(df, case, scenario, min_lat=None, max_lat=None,min_lon=None, max_lon=None):
    ...
    if min_lat is not None and max_lat is not None:
        ax.set_yticks(np.arange(np.floor(min_lat), np.ceil(max_lat), 0.3))
    if min_lon is not None and max_lon is not None:
        ax.set_xticks(np.arange(np.floor(min_lon), np.ceil(max_lon), 0.3))
...
if selected_page == 'Introduction':
    ...
    if subpage_selected == 'Prediction':
        ...
        # Generate map for Business-as-Usual
        plot1= generate_hab_quotient_map(selected_case['df'], selected_case, scenario='Business-as-Usual',min_lat=30, max_lat=30.5, min_lon=-85.9, max_lon=-85.3)
        # Display plots side by side using columns layout
        col1, col2 = st.columns(2)
        with col1:
            st.write("Plot 1")
